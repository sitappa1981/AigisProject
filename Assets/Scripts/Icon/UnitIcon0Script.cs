...
public class UnitIcon0Script : MonoBehaviour {

    public GameObject Popup00;  //ポップアップウィンドウの登録場所
    ...
    public void SkillText() {
        switch (ButtleScripts.PlayerNo[0]) {
            //ソーマ
            case 1001:
                //キャラ名
                Unit_Name.text = "ソーマ";
    ...
    void OnMouseDown() {
        //スキル使用後の場合は発動しない
        if (ButtleScripts.SkillCheck[0]) return;
        //プレイヤー0の所のキャラクターによってスキルが変動
        switch (ButtleScripts.PlayerNo[0]) {
            //ソーマの場合
            case 1001:
                //敵に直接ダメージを与える(一瞬)
            ...
            case 1002:
                ...
                StatusInformation.PhylisBuf[0] = (int)(ButtleScripts.PlayerST[0, 2] / 10);
                //自分のバフ計算に追加する
                ButtleScripts.Buff[0, 0] += StatusInformation.PhylisBuf[0];
                //効果時間は10秒
                Timer = 10.0f;
                //スキル発動判定を真にする
                SkillTime = true;

                Debug.Log(ButtleScripts.Buff[0, 0]);
            ...
            case 1003:
                ...
                StatusInformation.BeruniceBuff[0] = (int)(ButtleScripts.PlayerST[0, 3] / 10);
                //全員のバフ計算に追加する
                ButtleScripts.Buff[0, 1] += StatusInformation.BeruniceBuff[0];
                ButtleScripts.Buff[1, 1] += StatusInformation.BeruniceBuff[0];
                ButtleScripts.Buff[2, 1] += StatusInformation.BeruniceBuff[0];
            ...
            case 1004:
                ...
                //キャラクターのバフ量を算出して追加格納しておく
                StatusInformation.ValerieBuff[0] = (int)(ButtleScripts.PlayerST[0, 4] / 10);
        ...
        SkillText();
        //スキル使用判定を真にする
        ButtleScripts.SkillCheck[0] = true;
    ...
    void BuffOut() {
        ...
        //バフの加算量は0にする
        StatusInformation.PhylisBuf[0] = 0;
        StatusInformation.BeruniceBuff[0] = 0;
        StatusInformation.ValerieBuff[0] = 0;
